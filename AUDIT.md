...
with `SET sql_mode = ''` (D0, `src/BC/DatabaseConnectionFactory.php:70,133`) —
strict mode off, so truncation, zero-dates, and coerced values enter silently.
On top of that root cause: 318 columns are `NOT NULL DEFAULT ''`, so "present"
≠ "has a value" (D1); nothing prevents the same human existing as multiple
...
# Part 0 — AI-impact prioritization

> The AI-relevant subset of the five-part findings below, re-ranked **most →
> least critical to the clinical co-pilot**. Read format per item: **finding →
> why it matters to the AI → action / roadmap phase.** "Phase N" pointers
> refer to the build phases in [`ARCHITECTURE.md`](ARCHITECTURE.md) §7.
> (Formerly `docs/onboarding/AGENT-IMPL-AUDIT.md`, merged here unchanged.)
...
# Parts 1–5 — the full findings

> The complete five-audit record follows, unchanged from the former
> `docs/onboarding/AUDIT_FULL.md` (merged here): **Part 1 Security · Part 2 Performance ·
> Part 3 Architecture · Part 4 Data quality · Part 5 Compliance & regulatory**
> (each "Audit N" heading below is the corresponding part).
